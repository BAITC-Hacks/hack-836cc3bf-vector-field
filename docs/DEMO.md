...
| 0:40–1:20 | Выбрать «Вся сеть» и задать вопрос «Какие узлы наиболее приоритетны для дальнейшей проверки и почему?» | при разрешённом AI видны реальные tools и факты; без AI честный unavailable и рабочая очередь |
...
| 2:20–3:10 | Найти `100000002578405100`, затем «Расширить окружение до 200 узлов» и «Всё окружение» | распределитель со 116 получателями; другая weak component из 270 узлов; hidden count честный |
...
Резервный изолят: `100000000456947100` — 0 связей, полная карточка, priority 0 как пробел наблюдения. Неизвестный: `999999999999999999` → ENTITY_NOT_FOUND. Для глобального вопроса выбрать «Вся сеть»; для вопроса по текущему узлу — «Выбранный узел». Первый узел теперь открывается автоматически, но режим вопроса по умолчанию — вся сеть.
